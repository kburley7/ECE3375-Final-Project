...
void reveal(int x, int y) {
    if (x < 0 || y < 0 || x >= BOARD_SIZE || y >= BOARD_SIZE || revealed[y][x] || flagged[y][x]) return;
    revealed[y][x] = 1;
    if (board[y][x] == ' ')
        for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
...
void game_loop() {
    ...
    while (1) {
        ...
        if (keys & 0x2 && cx < BOARD_SIZE - 1) { cx++; wait_for_key_release(); }
        if (keys & 0x4 && cx > 0) { cx--; wait_for_key_release(); }
        if (keys & 0x8 && cy > 0) { cy--; wait_for_key_release(); }
